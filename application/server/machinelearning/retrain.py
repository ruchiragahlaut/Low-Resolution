...
def model_selector(X, y):
  ...
  top_models = [index for index, _ in sorted(enumerate(accuracies), key=lambda x: x[1], reverse=True)][:5]
  voting_clf = VotingClassifier([(str(i), models[i]) for i in top_models], voting='hard')
